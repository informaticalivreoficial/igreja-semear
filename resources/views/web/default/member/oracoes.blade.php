    ...
    <section class="bg-slate-50 py-12">
        <div class="container-site flex flex-col gap-8 lg:flex-row">
            ...
            <div class="min-w-0 flex-1 space-y-6">
                <form method="POST" action="{{ route('member.oracoes.store') }}" class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                    @csrf
                    <h2 class="font-display text-lg font-bold text-slate-900">Novo pedido</h2>
                ...
                <div class="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
                    <h2 class="font-display text-lg font-bold text-slate-900">Meus pedidos</h2>
                            ...
                            <div class="rounded-xl border border-slate-100 p-4">
                                ...
                                <div class="mt-3 flex items-center justify-between">
                                    ...
                                    @if($pedido->status === 'respondido' && $pedido->answer)
                                        <span class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-semibold text-emerald-700">Respondido</span>
                                    @else
