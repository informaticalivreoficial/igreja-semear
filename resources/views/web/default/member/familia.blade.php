            ...
            <div class="min-w-0 flex-1">
                @if($familia)
                    <div class="rounded-2xl border border-brand-100 bg-white p-6 shadow-sm">
                        <div class="flex items-center gap-3">
                            <div class="flex h-12 w-12 items-center justify-center rounded-2xl bg-brand-600/10 text-brand-700">
                                <svg class="h-6 w-6" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
                            </div>
                            <div>
                                <h2 class="font-display text-xl font-bold text-brand-900">{{ $familia->name }}</h2>
                            ...
                            @foreach($familia->members->sortBy([
                                ...
                                @php
                                    ...
                                    $cor = match ($pessoa->family_role) {
                                        'chefe' => 'bg-brand-600/10 text-brand-700',
                                        'conjuge' => 'bg-accent-500/15 text-accent-700',
                                        'filho' => 'bg-sky-600/10 text-sky-700',
                                        default => 'bg-slate-100 text-slate-600',
                                    };
                                @endphp
                                <div class="flex items-center gap-4 rounded-xl border border-brand-100 p-4 {{ $pessoa->id === $member->id ? 'ring-2 ring-brand-200' : '' }}">
                                    <div class="flex h-11 w-11 items-center justify-center rounded-full bg-brand-100 font-bold text-brand-700">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="truncate text-sm font-semibold text-slate-800">
                                            ...
                                            @if($pessoa->id === $member->id)
                                                <span class="text-xs font-medium text-brand-600">(você)</span>
                                            @endif
                                        </p>
                ...
                @else
                    <div class="rounded-2xl border border-brand-100 bg-white p-12 text-center shadow-sm">
                        <p class="font-display text-xl font-bold text-brand-900">Você ainda não está vinculado(a) a uma família</p>
