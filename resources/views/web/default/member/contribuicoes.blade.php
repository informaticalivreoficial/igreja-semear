    ...
    <section class="bg-brand-50 py-12">
        <div class="container-site flex flex-col gap-8 lg:flex-row">
            ...
            <div class="min-w-0 flex-1">
                <div class="mb-6 rounded-2xl bg-brand-700 p-6 text-white shadow-sm">
                    <p class="text-sm font-semibold text-brand-100">Total contribuído</p>
                    <p class="font-display mt-1 text-3xl font-bold">R$ {{ number_format($total, 2, ',', '.') }}</p>
                    <p class="mt-1 text-xs text-brand-100/80">{{ $doacoes->count() }} {{ $doacoes->count() === 1 ? 'registro' : 'registros' }}</p>
                </div>

                @forelse($doacoes as $doacao)
                    <div class="mb-3 flex items-center justify-between rounded-2xl border border-brand-100 bg-white p-5 shadow-sm">
                        <div>
                        ...
                        <div class="text-right">
                            <p class="text-sm font-bold text-brand-700">R$ {{ number_format($doacao->amount, 2, ',', '.') }}</p>
                ...
                @empty
                    <div class="rounded-2xl border border-brand-100 bg-white p-12 text-center shadow-sm">
                        <p class="font-display text-xl font-bold text-brand-900">Nenhuma contribuição registrada</p>
